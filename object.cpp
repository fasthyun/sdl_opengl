...
particle::particle():xObject()
{
    ...
    for ( int i=0 ; i < d_size ;i++ )
    {
        ...
        y=0; //rand()%1000 - 500;
        z=0; //rand()%1000 - 500;
        vertexData[i*3+0]=x;  // x
        vertexData[i*3+1]=y;  // y
        vertexData[i*3+2]=z;  // z
...
void particle::update_VBO()
{

    glBindVertexArray(VAO);
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * d_size /* bytes */, vertexData, GL_DYNAMIC_DRAW);
...
void particle::update(float dt)
{
    for ( int i=0 ; i < d_size ;i++ )
    {
        ...
        vertexData[i*3+0] += motes[i].force.x *dt;
        vertexData[i*3+1] += motes[i].force.y *dt;
        vertexData[i*3+2] += motes[i].force.z *dt;
        ///motes[i].force;
       // printf("x=%f  y=%f  z=%f \n",x,y,z);
        motes[i].life -= dt;
    }
