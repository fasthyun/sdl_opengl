...
//TODO: (hyun)  떼오는거 힘들겠는뎅..
void do_mode_triangle(tinygltf::Model model)
{
...
bool LoadGLTF(const string &filename, float scale,
  ...
  for (auto &gltfMesh : model.meshes) {
    ...
    MeshFloat loadedMesh(sizeof(float) * 3); //왜 3개 --> stride란거 3개라고 알려줌
    ...
    for (const auto &mesh : gltfMesh.primitives) {
      ...
      auto byteStride = indicesAccessor.ByteStride(bufferView);
      auto count = indicesAccessor.count;

      // This permit to get a type agnostic way of reading the index buffer
      // unique_ptr<intArrayBase> arrayPtr = nullptr;
      intArrayBase *arrayPtr = nullptr;

      // Allocate the index array in the pointer-to-base declared in the parent scope
      switch (indicesAccessor.componentType) {
      case TINYGLTF_COMPONENT_TYPE_BYTE:
          //arrayPtr = unique_ptr<intArray<char> >(new intArray<char>(arrayAdapter<char>(dataAddress, count, byteStride)));
          arrayPtr = new intArray<char>(arrayAdapter<char>(dataAddress, count, byteStride));
          break;

      case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE:
          arrayPtr =  new intArray<unsigned char>(arrayAdapter<unsigned char>(dataAddress, count, byteStride));
          break;

      case TINYGLTF_COMPONENT_TYPE_SHORT:
          arrayPtr =  new intArray<short>(arrayAdapter<short>(dataAddress, count, byteStride));
          break;

      case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT:
          arrayPtr =  new intArray<unsigned short>(arrayAdapter<unsigned short>(dataAddress, count, byteStride));
          break;

      case TINYGLTF_COMPONENT_TYPE_INT:
          arrayPtr = new intArray<int>(arrayAdapter<int>(dataAddress, count, byteStride));
          break;

      case TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT:
          arrayPtr = new intArray<unsigned int>(arrayAdapter<unsigned int>(dataAddress, count, byteStride));
          break;
      default:
          break;
      }

      const auto &indices = *arrayPtr; //  이거 vertex인덱스인듯 !

      if (arrayPtr) {
          cout << "indices " << arrayPtr->size() << "\n";
          for (size_t i(0); i < arrayPtr->size(); ++i) {
              // cout << indices[i] << " ";
              loadedMesh.faces.push_back(indices[i]); // 이건 인덱스인데...병신같다.
          }
          //cout << '\n';
      }
      ...
      switch (mesh.mode) {
          ...
          case TINYGLTF_MODE_TRIANGLES:  // this is the simpliest case to handle
              ...
              for (const auto &attr : mesh.attributes) {
                  ...
                  cout << " + has count " << count  << " and stride " << byte_stride << " bytes\n";
                  ...
                  // Face varying comment on the normals is also true for the UVs
                      ...
                      for (size_t i{0}; i < indices.size() / 3; ++i) {
                          ...
                          // get the texture coordinates for each triangle's vertices
                          v2f uv0, uv1, uv2;
                          uv0 = uvs[f0];
                          uv1 = uvs[f1];
                          uv2 = uvs[f2];
                  ...
                  {
                     cerr << " unknown attr.first : " << attr.first;
                  }
      ...
      // Material
      cout << "model.materials.len = "<< model.materials.size() << "\n";
      for (int i =0 ; i < model.materials.size(); i++ )
      {
          cout << "  + model.materials[0].name = "<< model.materials.at(i).name << "\n";
      }

      for (size_t i{0}; i < loadedMesh.faces.size(); ++i)
      {
         //cout << "model.material. idx "<< model.materials.size() << "\n";
  ...
  for (const auto &gltfTexture : model.textures) {
    ...
    cout << "Found texture: " << gltfTexture.source << gltfTexture.extras_json_string<< "\n";
  }
  return ret;
}
